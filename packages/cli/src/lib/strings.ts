...
import {Fingerprint} from '@pwawrap/core';
...
but received ${cyan(received.toString())}. Run ${cyan('pwawrap help')} for usage.`;
  },
...
If you are sure your project is updated and you have already run ${cyan('pwawrap update')}
...
\t  answered “yes”, PWAWrap uses the information to generate
...
\t  ${italic('theme_color')}. They will be used for notification icons.\n`,
  ...
  messageProjectBuildReminder: 'Build it by running ' + cyan('pwawrap build'),
...
PWAWrap will prompt for the creation of a new keystore.
...
The application generated by pwawrap will open a Progressive Web App when
...
\t\t- To open ${italic('https://example.com/path-to-pwa/')}: ${cyan('/path-to-pwa/')}\n`,
  ...
  promptInstallJdk: `Do you want PWAWrap to install JDK?
  (Enter "No" to use your JDK installation)`,
  promptJdkPath: 'Path to your existing JDK:',
  promptInstallSdk: `Do you want PWAWrap to install Android SDK?
  ...
  jdkPathIsNotCorrect: 'The jdkPath isn\'t correct, please run the following command to update ' +
      'it:\npwawrap updateConfig --jdkPath <path-to-jdk>, such that the folder of the path' +
      'contains the file "release". Then run pwawrap doctor again.',
  ...
  androidSdkPathIsNotCorrect: 'The androidSdkPath isn\'t correct, please run the following ' +
      'command to update it:\npwawrap updateConfig --androidSdkPath <path-to-sdk>, such that ' +
      'the folder of the path contains the folder "build". Then run pwawrap doctor again.',
